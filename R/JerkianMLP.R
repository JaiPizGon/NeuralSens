...
#' function of each neurons layer.
#' @param der3actfunc \code{character} vector indicating the third derivative of the activation
#' function of each neurons layer.
...
#'                       maxit = iters)
#' # Try JerkianMLP
#' NeuralSens::JerkianMLP(nnetmod, trData = nntrData, plot = FALSE)
#' \donttest{
#' # Try JerkianMLP to calculate sensitivities with respect to output of hidden neurones
#' NeuralSens::JerkianMLP(nnetmod, trData = nntrData,
#'                              sens_origin_layer = 2,
#'                              sens_end_layer = "last",
...
#'
#' # Try JerkianMLP
#' NeuralSens::JerkianMLP(caretmod)
#'
#' ## Train h2o NNET --------------------------------------------------------------
...
#'
#' # Try JerkianMLP
#' NeuralSens::JerkianMLP(RSNNSmod, trData = trData, output_name = "DEM")
#'
#' ## USE DEFAULT METHOD ----------------------------------------------------------
#' NeuralSens::JerkianMLP(caretmod$finalModel$wts,
#'                             trData = fdata.Reg.tr,
...
JerkianMLP <- function(MLP.fit,
                       ...
                       sens_end_input = FALSE,
                       ...) UseMethod('JerkianMLP', MLP.fit)

#' @rdname JerkianMLP
#'
#' @export
#'
#' @method JerkianMLP default
JerkianMLP.default <- function(MLP.fit,
                               .returnSens = TRUE,
                               plot = TRUE,
                               .rawSens = FALSE,
                               sens_origin_layer = 1,
  ...
  #    - D3 stores the third derivatives of the output values of each layer of neurons (Jerkian^l_l)
  #    - H stores the second derivatives of the output value of a layer with respect of two inputs (Hessian^l_p)
  #    - Q stores the second derivatives of the input value of a layer with respect of two inputs
  #    - D_ stores the derivatives of the output values with respect of one input (Jacobian^l_p)
  #    - J stores the third derivatives of the output value of a layer with respect to three inputs (Jerkian^l_p)
  #    - M stores the third derivatives of the input value of a layer with respect to three inputs
  Z <- list()
  O <- list()
  W <- list()
  D <- list()
  ...
  if (is.null(der3actfunc)) der3actfunc <- actfunc
  ...
  Der3ActivationFunction <- lapply(der3actfunc, NeuralSens::Der3ActFunc)
  ...
  D3[[1]] <- array(0, dim=c(mlpstr[1],mlpstr[1],mlpstr[1],mlpstr[1],nrow(TestData)))
  for (irow in 1:nrow(TestData)) {
    D[[1]][,,irow] <- DerActivationFunction[[1]](Z[[1]][irow,])
    D2[[1]][,,,irow] <- Der2ActivationFunction[[1]](Z[[1]][irow,])
    D3[[1]][,,,,irow] <- Der3ActivationFunction[[1]](Z[[1]][irow,])
  }
  ...
  for (l in 2:length(mlpstr)){
    ...
    D3[[l]] <- array(diag(mlpstr[l]),dim=c(mlpstr[l],mlpstr[l],mlpstr[l],mlpstr[l],nrow(TestData)))
    for (irow in 1:nrow(TestData)) {
      D[[l]][,,irow] <- DerActivationFunction[[l]](Z[[l]][irow,])
      D2[[l]][,,,irow] <- Der2ActivationFunction[[l]](Z[[l]][irow,])
      D3[[l]][,,,,irow] <- Der3ActivationFunction[[l]](Z[[l]][irow,])
    }
  }
...
#' @export
#'
#' @method JerkianMLP train
JerkianMLP.train <- function(MLP.fit,
                             .returnSens = TRUE,
                             plot = TRUE,
                             .rawSens = FALSE,
                             sens_origin_layer = 1,
...
#' @export
#'
#' @method JerkianMLP H2OMultinomialModel
JerkianMLP.H2OMultinomialModel <- function(MLP.fit,
                                           .returnSens = TRUE,
                                           plot = TRUE,
                                           .rawSens = FALSE,
                                           sens_origin_layer = 1,
  ...
  PrepActFuncs <- function(acfun) {
    ...
    switch(acfun,
           ...
           Maxout = {
             stop("JerkianMLP function is not ready for Maxout layers")
           },
           MaxoutDropout = {
             stop("JerkianMLP function is not ready for Maxout layers")
           },
           ...
           {
             stop("JerkianMLP is not ready for the activation function used")
           })
  }
  actfun <- sapply(MLP.fit@model$model_summary$type, PrepActFuncs)
  # Call to the default function
  JerkianMLP.default(finalModel,
                     trData = trData,
                     actfunc = actfun,
                     .returnSens = .returnSens,
                     .rawSens = .rawSens,
                     ...
                     der3actfunc = if("der3actfunc" %in% names(args)){args$der3actfunc}else{NULL},
                     args[!names(args) %in% c("trData","output_name","deractfunc","der2actfunc","der3actfunc")])
}
...
#' @export
#'
#' @method JerkianMLP H2ORegressionModel
JerkianMLP.H2ORegressionModel <- function(MLP.fit,
                                          .returnSens = TRUE,
                                          plot = TRUE,
                                          .rawSens = FALSE,
                                          sens_origin_layer = 1,
...
#' @export
#'
#' @method JerkianMLP list
JerkianMLP.list <- function(MLP.fit,
                            .returnSens = TRUE,
                            plot = TRUE,
                            .rawSens = FALSE,
                            sens_origin_layer = 1,
  ...
  JerkianMLP.default(finalModel,
                     ...
                     der2actfunc = if("der2actfunc" %in% names(args)){args$der2actfunc}else{NULL},
                     der3actfunc = if("der3actfunc" %in% names(args)){args$der3actfunc}else{NULL},
                     args[!names(args) %in% c("output_name","deractfunc","der2actfunc","der3actfunc")])
}
...
#' @export
#'
#' @method JerkianMLP mlp
JerkianMLP.mlp <- function(MLP.fit,
                           .returnSens = TRUE,
                           plot = TRUE,
                           .rawSens = FALSE,
                           sens_origin_layer = 1,
...
#' @export
#'
#' @method JerkianMLP nn
JerkianMLP.nn <- function(MLP.fit,
                          .returnSens = TRUE,
                          plot = TRUE,
                          .rawSens = FALSE,
                          sens_origin_layer = 1,
  ...
  for (j in 1:length(MLP.fit$weights)) {
    ...
    finalModel$wts <- wts
    sensitivities <- JerkianMLP.default(finalModel,
                                        trData = trData,
                                        actfunc = actfun,
                                        .returnSens = TRUE,
                                        .rawSens = TRUE,
...
#' @export
#'
#' @method JerkianMLP nnet
JerkianMLP.nnet <- function(MLP.fit,
                            .returnSens = TRUE,
                            plot = TRUE,
                            .rawSens = FALSE,
                            sens_origin_layer = 1,
...
#' @export
#'
#' @method JerkianMLP nnetar
JerkianMLP.nnetar <- function(MLP.fit,
                              .returnSens = TRUE,
                              plot = TRUE,
                              .rawSens = FALSE,
                              sens_origin_layer = 1,
  ...
  for (i in 1:length(MLP.fit$model)) {
    ...
    sensitivities[[i]] <-  JerkianMLP.default(finalModel,
                                              trData = trData,
                                              actfunc = actfun,
                                              deractfunc = if("deractfunc" %in% names(args)){args$deractfunc}else{NULL},
                                              der2actfunc = if("der2actfunc" %in% names(args)){args$der2actfunc}else{NULL},
...
#' @export
#'
#' @method JerkianMLP numeric
JerkianMLP.numeric <- function(MLP.fit,
                               .returnSens = TRUE,
                               plot = TRUE,
                               .rawSens = FALSE,
                               sens_origin_layer = 1,
