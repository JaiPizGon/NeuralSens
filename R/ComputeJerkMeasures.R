...
ComputeJerkMeasures <- function(sens) {
  ...
  D3 <- sens$layer_third_derivatives
  W <- sens$mlp_wts
  D_[[1]] <- sens$layer_derivatives[[sens_origin_layer]]
  Q[[1]] <- diag3Darray(dim = mlpstr[sens_origin_layer])
  H[[1]] <- D2[[sens_origin_layer]]
  M[[1]] <- diag4Darray(dim = mlpstr[sens_origin_layer])
  J[[1]] <- D3[[sens_origin_layer]]
  ...
  if (sens_origin_layer != sens_end_layer) {
    ...
    for (l in (sens_origin_layer+1):sens_end_layer) {
      ...
      H[[counter]] <- array(NA, dim=c(mlpstr[sens_origin_layer], mlpstr[l], mlpstr[sens_origin_layer], nrow(TestData)))
      M[[counter]] <- array(NA, dim=c(mlpstr[sens_origin_layer], mlpstr[l], mlpstr[sens_origin_layer], mlpstr[sens_origin_layer], nrow(TestData)))
      J[[counter]] <- array(NA, dim=c(mlpstr[sens_origin_layer], mlpstr[l], mlpstr[sens_origin_layer], mlpstr[sens_origin_layer], nrow(TestData)))
      for (irow in 1:nrow(TestData)) {
        ...
        Q[[counter]][,,,irow] <- array(apply(array(H[[counter - 1]][,,,irow,drop=FALSE], dim = dim(H[[counter - 1]])[1:3]), 3,
                                             function(x) x %*% matrix(W[[l]][2:nrow(W[[l]]),,drop=FALSE],
                                                                      nrow = dim(W[[l]])[1] - 1,
                                                                      ncol = dim(W[[l]])[2])),
                                       dim = c(mlpstr[sens_origin_layer], dim(W[[l]])[2], mlpstr[sens_origin_layer]))

        H[[counter]][,,,irow] <- array(apply(array(apply(array(D2[[l]][,,,irow], dim = dim(D2[[l]])[1:3]), 3,
